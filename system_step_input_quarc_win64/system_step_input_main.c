...
typedef RT_MODEL_system_step_input_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * system_step_input(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(system_step_input_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: system_step_input -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
