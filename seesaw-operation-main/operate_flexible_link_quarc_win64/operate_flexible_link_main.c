...
typedef RT_MODEL_operate_flexible_lin_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * operate_flexible_link(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(operate_flexible_link_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: operate_flexible_link -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
